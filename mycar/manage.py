...
Usage:
    manage.py (drive) [--model=<model>] [--js] [--type=(linear|categorical|myflows)] [--camera=(single|stereo)] [--meta=<key:value> ...] [--myconfig=<filename>]
...
from mycar_web import MycarLocalWebController
...
def drive(cfg, model_path=None, use_joystick=False, model_type=None,
    ...
    # Keras/TF 与 Unity 模拟器争用 GPU；MyFlows ONNX 推理可走 CUDA/DirectML。
    if cfg.DONKEY_GYM and model_type != "myflows":
    ...
    #
    # Optional: use web buttons (1~5) as discrete steering control.
    # This is a reliable fallback when the on-screen joystick fails to send angle updates on some Windows setups.
    if getattr(cfg, "WEB_BUTTON_STEERING_ENABLE", False):
        class WebButtonSteering:
            def __init__(self):
                self.last = 0.0

            def run(self, w1, w2, w3, w4, w5, steering):
                # buttons are latched booleans; prioritize strongest command
                if w1:
                    self.last = -1.0
                elif w2:
                    self.last = -0.5
                    ...
                    # if no button pressed, pass through existing steering
                    if steering is not None:
                        self.last = float(steering)
                return float(self.last)

        V.add(
            WebButtonSteering(),
            inputs=["web/w1", "web/w2", "web/w3", "web/w4", "web/w5", "user/steering"],
    ...
    # 网页/摇杆/键盘指令统一在此钳位（与 myconfig 中 WEB_MAX_THROTTLE 一致）
    web_max_t = max(0.0, min(1.0, float(getattr(cfg, "WEB_MAX_THROTTLE", 1.0))))
    if web_max_t < 1.0:
        V.add(
            Lambda(
                lambda t, c=web_max_t: t
                if t is None
                else c
                if float(t) > c
                else (-c if float(t) < -c else float(t))
            ),
            inputs=["user/throttle"],
            outputs=["user/throttle"],
        )

    # Web UI sometimes fails to send throttle updates (or stays near zero).
    # Provide a small default throttle in user mode so the car can actually move in sim.
    web_default_throttle = getattr(cfg, "WEB_DEFAULT_THROTTLE", 0.0)
    if web_default_throttle and web_default_throttle > 0.0:
        V.add(
            Lambda(
                lambda t, m: float(web_default_throttle)
                if (m == "user" and (t is None or abs(float(t)) < 0.02))
                else t
            ),
    ...
    def load_model_json(kl, json_fnm):
        ...
        from tensorflow.python import keras  # pyright: ignore[reportMissingImports]
        try:
    ...
    #
    if model_path and model_type == "myflows":
        from myflows_pilot import MyFlowsResNet18Pilot

        kl = MyFlowsResNet18Pilot(
            model_path,
            fixed_throttle=getattr(cfg, "MYFLOWS_FIXED_THROTTLE", 0.5),
            max_throttle=getattr(cfg, "MYFLOWS_MAX_THROTTLE", getattr(cfg, "WEB_MAX_THROTTLE", 0.5)),
            steering_scale=getattr(cfg, "MYFLOWS_STEERING_SCALE", 1.0),
            debug=getattr(cfg, "MYFLOWS_DEBUG", False),
            device=getattr(cfg, "MYFLOWS_DEVICE", "auto"),
        )
        from pathlib import Path as _Path
        _mp = _Path(model_path)
        watch_path = str(_mp) if _mp.suffix else str(_mp.with_suffix(".json"))
        V.add(FileWatcher(watch_path, verbose=True),
              outputs=['modelfile/modified'])
        V.add(kl, inputs=['cam/image_array'],
              outputs=['pilot/angle', 'pilot/throttle'],
              run_condition='run_pilot')
        # MyFlows 已接入 pilot，跳过下面 Donkey/Keras 的模型加载逻辑。
        model_path = None
...
def add_user_controller(V, cfg, use_joystick, input_image='ui/image_array'):
    ...
    #
    ctr = MycarLocalWebController(port=cfg.WEB_CONTROL_PORT, mode=cfg.WEB_INIT_MODE)
    V.add(ctr,
...
def add_simulator(V, cfg):
    ...
    if cfg.DONKEY_GYM:
        # DonkeyCar v5.3.0 的 DonkeyGymEnv 还在用老 gym API，而新版 gym-donkeycar 已迁移到 gymnasium。
        # 这里直接用 gymnasium + gym_donkeycar（import 时完成 env 注册），避免版本不兼容导致的 “env 不存在 / conf 参数报错”。
        import gymnasium as gym
        import numpy as np
        import gym_donkeycar  # noqa: F401  (import triggers gymnasium env registration)

        class GymnasiumDonkeyGymEnv(object):
            def __init__(self, sim_path, host="127.0.0.1", port=9091, env_name="donkey-generated-track-v0", conf=None, delay=0):
                conf = {} if conf is None else dict(conf)
                conf["exe_path"] = sim_path
                conf["host"] = host
                conf["port"] = port
                conf.setdefault("frame_skip", 1)
                self.env = gym.make(env_name, conf=conf)
                obs, info = self.env.reset()
                self.frame = obs
                self.info = info or {}
                # gym_donkeycar (gymnasium) expects action shape (2,): [steer, throttle]
                self.action = [0.0, 0.0]
                self.running = True
            ...
            def update(self):
                while self.running:
                    step_out = self.env.step(self.action)
                    # gymnasium: (obs, reward, terminated, truncated, info)
                    obs, _, _, _, info = step_out
                    if self.delay > 0.0:
                        self.delay_buffer(obs, info)
                    else:
                        self.frame = obs
                        self.info = info
            ...
            def run_threaded(self, steering, throttle, brake=None):
                if steering is None or throttle is None:
                    steering = 0.0
                    throttle = 0.0
                self.action = [float(steering), float(throttle)]
                # threaded=True 时 Vehicle 只调用 run_threaded，并把它当作 outputs 写入 mem。
                # 不 return 时 cam/image_array 永为 None，Tub 里只会记 angle/throttle 没有图。
                if self.frame is None:
                    return None
                return np.ascontiguousarray(self.frame)

            def run(self, steering, throttle, brake=None):
                return self.run_threaded(steering, throttle, brake=brake)
