var driveHandler = new function() {
    ...
    // WASD：固定油门/固定转角/松手归零，持键 25ms 重发；MYCAR_DEBUG_WS 开才有 WS 详细日志
    var MYCAR_DEBUG_WS = false
    var wasdKeyDown = { w: false, a: false, s: false, d: false }
    var wasdStreamTimer = null
    // W=前进油门占 maxThrottle 比例；S=倒车；A/D=转角(±1)
    var MYCAR_KEY_W_THROTTLE_PCT = 0.85
    var MYCAR_KEY_S_THROTTLE_PCT = 0.65
    // A/D 固定转角(±1 为满舵)；急弯可保持 0.95~1.0，若发飘可略降到 0.85
    var MYCAR_KEY_AD_ANGLE = 0.95
    var MYCAR_WASD_TICK_MS = 25

    this.load = function() {
      driveURL = '/drive'
      console.log('mycar web_static: 固定WASD+低延迟(25ms重发)/关闭全量console 已加载');
      socket = new WebSocket('ws://' + location.host + '/wsDrive');

      setBindings();
      registerMycarWasdKeyboard();
      ...
      if ($('#max_throttle_select').length) {
        $('#max_throttle_select').val(String(state.maxThrottle));
      }
    ...
    var setBindings = function() {
      ...
      socket.onmessage = function (event) {
        if (MYCAR_DEBUG_WS) { console.log(event.data); }
      ...
      // WASD 固定车控在 registerMycarWasdKeyboard() 里注册，避免与 IJKL 的增量调参混在一起
      $(document).keydown(function(e) {
          ...
          if (isJ) { angleLeft() }
          if (isL) { angleRight() }
          if(e.which == 50) { updateDriveMode('local') }  // '2' full local autopilot
          if(e.which == 51) { updateDriveMode('local_angle') }  // '3' local auto steering
          if(c === 'KeyU' || e.which == 85) { updateDriveMode('user') } // 'u' user
          if(c === 'KeyM' || e.which == 77) { toggleDriveMode() } // 'm' toggle mode
      });
    ...
    function bindNipple(manager) {
      manager.on('start', function(evt, data) {
        ...
        //console.log(data)
        // 用稍大的分母（原 70）让虚拟摇杆同样行程下更“肉”、油门更小
        var stickDiv = 100
        state.tele.user.angle = Math.max(Math.min(Math.cos(radian)/stickDiv*distance, 1), -1)
        state.tele.user.throttle = limitedThrottle(Math.max(Math.min(Math.sin(radian)/stickDiv*distance , 1), -1))
    ...
    var postDrive = function(fields=[]) {
        ...
        if(data) {
            ...
            if (MYCAR_DEBUG_WS) { console.log(`Posting ${json_data}`); }
    ...
    function wasdAny() {
      return wasdKeyDown.w || wasdKeyDown.a || wasdKeyDown.s || wasdKeyDown.d;
    }

    function applyWASDToState() {
      state.brakeOn = false;
      var t = 0;
      var a = 0;
      if (wasdKeyDown.w && !wasdKeyDown.s) {
        t = state.maxThrottle * MYCAR_KEY_W_THROTTLE_PCT;
      } else if (wasdKeyDown.s && !wasdKeyDown.w) {
        t = -state.maxThrottle * MYCAR_KEY_S_THROTTLE_PCT;
      }
      if (wasdKeyDown.a && !wasdKeyDown.d) {
        a = -MYCAR_KEY_AD_ANGLE;
      } else if (wasdKeyDown.d && !wasdKeyDown.a) {
        a = MYCAR_KEY_AD_ANGLE;
      }
      state.tele.user.throttle = limitedThrottle(t);
      state.tele.user.angle = Math.max(-1, Math.min(1, a));
    }

    function stopWASDStream() {
      if (wasdStreamTimer) {
        clearInterval(wasdStreamTimer);
        wasdStreamTimer = null;
      }
    }

    function startWASDStream() {
      if (wasdStreamTimer) {
        return;
      }
      if (!wasdAny()) {
      ...
      wasdStreamTimer = setInterval(function() {
        applyWASDToState();
        postDrive();
      }, MYCAR_WASD_TICK_MS);
    }

    function resetWASDKeys() {
      wasdKeyDown.w = wasdKeyDown.a = wasdKeyDown.s = wasdKeyDown.d = false;
      stopWASDStream();
    }

    function wasdKeyCodeFromEvent(e) {
      var c = e.code;
      if (c) {
        return c;
      }
    ...
    function registerMycarWasdKeyboard() {
      $(document).off('keydown.mycarFixed keyup.mycarFixed');
      $(window).off('blur.mycarFixed');

      $(document).on('keydown.mycarFixed', function(e) {
        var c = wasdKeyCodeFromEvent(e);
      ...
      $(document).on('keyup.mycarFixed', function(e) {
        var c = wasdKeyCodeFromEvent(e);
        if (c !== 'KeyW' && c !== 'KeyA' && c !== 'KeyS' && c !== 'KeyD') {
          return;
        }
      ...
      $(window).on('blur.mycarFixed', function() {
        if (!wasdAny()) {
          return;
        }
        resetWASDKeys();
