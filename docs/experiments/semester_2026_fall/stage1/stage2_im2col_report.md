...
3. `native_cublas_report.md`：自写 CUDA 重排、原生 C/C++ 调度和 cuBLAS 的成功对照；P0/P1/P2 的组合耗时均优于 CuPy。
4. 后续自写 GEMM 优化成功数据：只有在相同 fixture、相同顺序、重复运行并确认大部分目标场景优于 CuPy 后才新增。
...
原生 C/C++ 调度路径已完成首轮实验。`cuda_native_cublas` 保留自写 im2col/col2im，但由项目自己的原生 DLL 在同一 stream 上启动 kernel 并调用 cuBLAS；P0/P1/P2 的组合耗时为 `0.315/0.282/0.258 ms`，CuPy 为 `0.856/0.704/0.753 ms`。这证明了“自写 CUDA 算子 + 官方 CUDA 库 + C/C++ 调度”可以作为区别于 CuPy 封装路径的有效实现；详细表格见 [原生 C/C++ 调度报告](native_cublas_report.md)。

后续正式表统一使用中文列名，至少包括场景、前向、反向、合计、传输计时、中位数、P95 和结论。`P0/P1/P2`、CUDA、CuPy、GEMM、Nsight、FP32 等专有名词保留原写法。

## 下一步

下一步用 Nsight Compute 分析原生 cuBLAS 路径的 kernel 数量、im2col 写入和 stream 调度，再分析自写 GEMM 的占用率、全局内存访问、共享内存冲突和 kernel 启动开销。若自写 GEMM 在多数场景仍慢，继续保留它作为失败尝试；当前默认候选是 `cuda_native_cublas`，但仍需在更多 batch、通道和矩阵规模下复测。
